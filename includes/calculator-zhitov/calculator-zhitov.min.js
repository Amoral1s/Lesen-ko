...
jQuery(document).ready(function(a){var c=function(e){var d=e.find("form"),f=e.find(".block_raschet_r");f.html("");f.addClass("loading");a.ajax({url:"/wp-admin/admin-ajax.php?action=zhitov_calculator&data-lestnica="+(d.is("[data-lestnica]")?d.attr("data-lestnica"):""),type:"get",data:d.serialize(),success:function(g){g=a(g);f.removeClass("loading");f.append(g.next("div.sp_calc"));f.append(g.next("div#drawing"));f.append(g.next("div.inline"))},done:function(){f.removeClass("loading").html("<h1>Error. Try again</h1>")}})},
b=a(".calculator-zhitov ");0<b.length&&(b.find("form").on("submit",function(e){e.preventDefault();c(b)}),c(b))});
